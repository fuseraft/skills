...
This skill currently implements the shared helper (`scripts/_ado_common.ps1`), foundation scripts, and REST-first scripts for work items, pull requests, repository listing, code search, and pipeline run listing (see "Current scripts" above). The `-Backend cli` option is accepted by all scripts but not yet implemented; every script currently requires `-Backend rest`. `-Backend auto` resolves to `rest` only when the Azure CLI (`az`) is not installed: when `az` is on `PATH`, `auto` prefers the CLI and the scripts stop with "currently supports only the REST backend", so pass `-Backend rest` on machines that have it.
