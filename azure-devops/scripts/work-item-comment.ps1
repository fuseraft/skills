<#
.SYNOPSIS
Adds a comment to an Azure DevOps work item through REST.
...
.EXAMPLE
pwsh -File scripts/work-item-comment.ps1 -Id 48520 -Comment "Deployed to staging."
#>
[CmdletBinding()]
param(
    [string]$Org,
...
try {
    ...
    $response = Invoke-AdoRest -Context $context -Method POST -Project $context['project'] `
        -Path "_apis/wit/workItems/$Id/comments" -Body ([ordered]@{ text = $Comment }) -ApiVersion '7.1-preview.4'

    $commentData = $response['data']
    if ($null -eq $commentData) { $commentData = [ordered]@{} }
    $payload = [ordered]@{
        ...
        request   = [ordered]@{
            id      = (Get-AdoItem $commentData 'workItemId')
            comment = $Comment
        }
        comment   = $commentData
    }
