<#
.SYNOPSIS
Creates an Azure DevOps work item through REST using JSON Patch.

.DESCRIPTION
-Description and -AcceptanceCriteria are HTML rich-text fields in Azure DevOps, where a
literal newline renders as nothing. Pass plain text with real blank lines between
paragraphs and "- " / "* " for bullet lines; it is converted to <p>/<br>/<ul> HTML before
sending. Text that already contains "<" is sent unchanged.

.PARAMETER Type
Work item type, for example Bug, Task, or "User Story" (required).
...
.EXAMPLE
pwsh -File scripts/work-item-create.ps1 -Type Bug -Title "Example bug" -Field Microsoft.VSTS.Common.Priority=1
#>
[CmdletBinding()]
param(
    [string]$Org,
...
Assert-AdoRequiredParameter -Bound $PSBoundParameters -Name 'Type', 'Title'
...
try {
    ...
    if ($resolvedBackend -ne 'rest') { throw (New-AdoError 'work-item-create.ps1 currently supports only the REST backend') }
    ...
    $requested = [ordered]@{ 'System.Title' = $Title }
    $operations.Add([ordered]@{ op = 'add'; path = '/fields/System.Title'; value = $Title })

    if ($PSBoundParameters.ContainsKey('Description')) {
        $html = ConvertTo-AdoHtml -Text $Description
        $operations.Add([ordered]@{ op = 'add'; path = '/fields/System.Description'; value = $html })
        $requested['System.Description'] = $html
    }
    if ($PSBoundParameters.ContainsKey('AcceptanceCriteria')) {
        $html = ConvertTo-AdoHtml -Text $AcceptanceCriteria
        $operations.Add([ordered]@{ op = 'add'; path = '/fields/Microsoft.VSTS.Common.AcceptanceCriteria'; value = $html })
        $requested['Microsoft.VSTS.Common.AcceptanceCriteria'] = $html
    }
    if ($PSBoundParameters.ContainsKey('AssignedTo')) {
        $operations.Add([ordered]@{ op = 'add'; path = '/fields/System.AssignedTo'; value = $AssignedTo })
        $requested['System.AssignedTo'] = $AssignedTo
    }
    ...
    foreach ($raw in $assignments) {
        $parsed = ConvertFrom-FieldAssignment -Raw $raw
        $requested[$parsed.Name] = $parsed.Value
    }

    $response = Invoke-AdoRest -Context $context -Method POST -Project $context['project'] `
        -Path ('_apis/wit/workitems/$' + (ConvertTo-AdoQuote -Text $Type)) `
    ...
    $payload = [ordered]@{
        ...
        request   = [ordered]@{ type = $Type; fields = $requested }
        work_item = [ordered]@{
